...
print(basis_down(np.zeros(4), 0))
num_state = 4  #yoyo-z-pos, z-vel, rot-vel, ee-z-pos
num_basis_down = len(basis_down(np.zeros(4), 0))
km_down = Koopman(basis_down, num_basis_down, num_state)


def basis_up(state, action):
    #extra_basis = np.array([np.sin(state[0]), np.sin(state[1]),np.sin(state[2]),np.sin(state[3]),np.cos(state[0]), np.cos(state[1]),np.cos(state[2]),np.cos(state[3])])
    extra_basis = np.array([state[0]**2,action])
    psi = np.hstack((state, extra_basis))
    return psi


print(basis_up(np.zeros(4), 0))
num_state = 4  #yoyo-z-pos, z-vel, rot-vel, ee-z-pos
num_basis_up = len(basis_up(np.zeros(4), 0))
...
for i in range(2, len(args) - 2):
    ...
    down_data_group, down_action_group = dp.create_state_pair(down_motion, [1,2,4,5])
    up_data_group, up_action_group = dp.create_state_pair(up_motion, [1,2,4,5])
...
axs[4].plot(range(len(action_list)), action_list)
plt.show()
